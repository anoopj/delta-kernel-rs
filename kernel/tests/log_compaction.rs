...
#[tokio::test]
async fn test_log_compaction_e2e_simple() -> Result<(), Box<dyn std::error::Error>> {
...
"#,
    ...
    let mut writer = snapshot.get_log_compaction_writer(0, 2)?;
    ...
    let total_actions: i64 = compaction_data.total_actions();
    let add_actions: i64 = compaction_data.total_add_actions();
    println!("Total actions after processing: {}", total_actions);
    println!("Add actions after processing: {}", add_actions);

    // Verify expected action counts
    // Expected:
    // 1 protocol + 1 metadata + 1 remove = 3 total actions
    // 0 add actions (the original add was cancelled out by the remove)
    assert_eq!(
        total_actions, 3,
        "Expected 3 total actions (protocol + metadata + remove)"
    );
    assert_eq!(
        add_actions, 0,
        "Expected 0 add actions (add was cancelled by remove)"
    );

    // Convert the compaction data into JSON
